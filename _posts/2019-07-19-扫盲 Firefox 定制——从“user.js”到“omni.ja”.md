...
date: 2019-07-19T21:50:00+08:00
author: 编程随想的博客
from: https://program-think.blogspot.com/2019/07/Customize-Firefox.html
tags: [ 编程随想的博客 ]
categories: [ 编程随想的博客 ]
---
...
<div class="post hentry uncustomized-post-template">
 ...
 <div class="post-body entry-content">
  ...
  然后敲回车
  <br/>
  2. 这时候会显示一个支持页面，其中有一项
  <code>
   Profile Directory
  </code>
  （俺用的是英文版，中文版的叫法略有不同），对应的就是【当前实例的目录】。
  <br/>
  <br/>
  <br/>
  <h2>
  ...
  <br/>
  该搜索框针对 Preference 的名称进行【模糊匹配】。比如说：你在搜索框中输入
  <code>
   network
  </code>
  ，那么界面上就只列出名称中包含
  ...
  ，每次你打开“about:config”界面都会显示警告提示；反之，则不显示。
  <br/>
  ...
  的配置文件。这个文件保存了当前实例中【所有】Preferences 的设置。
  <br/>
  <br/>
  <h3>
   ◇文件位置
  ...
  ，说明其内容是 javascript 语法。
  <br/>
  在 javascript 语法中，任何一行中如果出现【双斜杠】
  <code>
   //
  </code>
  ...
  之间的内容也是【注释】。（注：第二种注释可以跨越多行）
  <br/>
  除了注释之外，该文件的每一行有效代码对应一个 Preferences。文件中的每个有效代码行全都是如下这个样子：
  <br/>
  <div class="source">
   <pre><span class="nx">user_pref</span><span class="p">(</span><span class="s2">"配置项名称"</span><span class="p">,</span> <span class="nx">配置项数值</span><span class="p">);</span>
</pre>
  </div>
  <br/>
  ...
  <br/>
  <div class="source">
   <pre><span class="nx">user_pref</span><span class="p">(</span><span class="s2">"general.warnOnAboutConfig"</span><span class="p">,</span> <span class="kc">false</span><span class="p">);</span>
</pre>
  </div>
  ...
  ，拿到这两个文件中的 Preferences 配置，并保存在内存中。
  <br/>
  如果发现这两个文件的配置有冲突，以
  <code>
   user.js
  ...
  <br/>
  如果你在这个界面上进行了修改，修改的结果会保存在内存中，同时也会保存到
  <code>
   prefs.js
  </code>
  文件中。
  <br/>
  另外，Firefox【不会】去修改你用于定制的
  <code>
   user.js
  </code>
  文件。
  ...
  <br/>
  （熟悉本博客的读者应该知道：俺一向鼓励【折腾】，并写过一篇博文《
  <a href="https://program-think.blogspot.com/2017/04/The-Importance-of-Zheteng.html">
   聊聊【折腾】的重要性
  </a>
  》）
  <br/>
  ...
  <br/>
  Mozilla 提供了名为【autoconfig】的机制，可以更进一步地定制 Preference。具体参见 Mozilla 官网的“
  <a href="https://support.mozilla.org/en-US/kb/customizing-firefox-using-autoconfig" rel="nofollow" target="_blank">
   这篇教程
  </a>
  ”（洋文）。
  <br/>
  ...
  <br/>
  <div class="source">
   <pre><span class="nx">defaultPref</span><span class="p">(</span><span class="s2">"配置项名称"</span><span class="p">,</span> <span class="nx">配置项数值</span><span class="p">);</span>  <span class="c1">// 该函数用于：定制配置项的【默认值】</span>
...
<span class="nx">clearPref</span><span class="p">(</span><span class="s2">"配置项名称"</span><span class="p">);</span>  <span class="c1">//  该函数用于：把配置项的【当前值】恢复为【默认值】</span>
  ...
  的结构，就可以对 Firefox 的界面进行任意定制。
  <br/>
  说到这里，某些熟悉 Firefox 的用户会反问：为啥不用
  <code>
   userChrome.css
  </code>
  定制 Firefox？
  <br/>
  这个东东俺当然知道（而且还写过专门的教程，链接在“
  <a href="https://program-think.blogspot.com/2016/10/custom-firefox-theme-without-extension.html">
   这里
  </a>
  ”）。但它的能力很【有限】——只能修改 CSS 样式，【无法】定制界面的功能。
  <br/>
  <br/>
  举个【简单】例子：
  <br/>
  ...
  ，但其实是个 zip 压缩包；但它又【不是】标准的 zip 压缩包——因为它的文件头有点特殊。
  <br/>
  俺在 Linux 下用
  <code>
   unzip
  </code>
  命令进行解压缩，完全 OK（使用 Mac OS 的同学也可以用这个命令）；
  <br/>
  对于使用 Windows 的同学，（Mozilla 官网的文档建议）先把扩展名改为
  <code>
   .zip
  </code>
  ...
  <br/>
  <pre class="shell">zip -qr9XD omni.ja *</pre>
  <br/>
  <h3>
   ◇定制“omni.ja”的注意事项
  </h3>
  ...
  会随着 Firefox 的版本而变化。也就是说，每次有新版本，你还需要在新版本上再修改一次。
  <br/>
  ...
  <div class="post-copyright">
   ...
   和本文原始地址：
   <br/>
   <a href="https://program-think.blogspot.com/2019/07/Customize-Firefox.html" id="OriginalPostUrl">
